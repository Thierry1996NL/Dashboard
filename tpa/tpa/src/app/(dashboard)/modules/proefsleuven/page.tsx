...
export default function ProefsleuvenPage() {
  ...
  const handleSave = async () => {
    if (!form.proefsleuv_nr?.trim()) { toast('Proefsleuv nr. is verplicht', 'error'); return; }
  ...
  return (
    <div className="page-content">
      ...
      <div className="controls-bar">
        ...
        <div style={{ flex: 1 }} />
        {mode === 'editor' && <button className="btn btn-primary" onClick={() => openModal()}>+ Proefsleuv toevoegen</button>}
      </div>
      <div className="table-wrap"><div className="tbl-scroll">
        <table>
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={10}><div className="empty-state"><strong>Geen proefsleuven gevonden</strong>Pas de filters aan.</div></td></tr>
            ) : rows.map(d => (
